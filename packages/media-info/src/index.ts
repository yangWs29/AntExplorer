...
export interface ParsedMediaInfo {
  ...
  filePath: string;
  /** 创建库信息 */
  creatingLibrary?: { name: string; version: string; url?: string };
  /** 通用信息 */
 ...
 * 4. PATH 中的 mediainfo
 */
function findMediaInfoBin(customPath?: string): string {
  // 1. 使用自定义路径
  if (customPath) {
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          reject(
            new Error(
              `mediainfo 执行失败: ${error.message}${stderr ? `\nstderr: ${stderr}` : ""}`,
            ),
          );
          return;
        }
 ...
 */
export function parseMediaInfoResult(
  raw: MediaInfoResult,
  filePath: string,
): ParsedMediaInfo {
  ...
  for (const track of raw.media.track) {
    const { "@type": type, ...rest } = track as Record<string, unknown> & {
      "@type": string;
    };
    switch (type) {
      case "General":
        result.general = rest as unknown as GeneralTrack;
        break;
      case "Video":
        result.video.push(rest as unknown as VideoTrack);
        break;
      case "Audio":
        result.audio.push(rest as unknown as AudioTrack);
        break;
      case "Text":
        result.text.push(rest as unknown as TextTrack);
        break;
      case "Image":
        result.image.push(rest as unknown as ImageTrack);
        break;
      case "Menu":
        result.menu.push(rest as unknown as MenuTrack);
        break;
    }
  }
 ...
 */
export async function getMediaInfoRaw(
  filePath: string,
  ...
  return JSON.parse(output) as MediaInfoResult;
}
 ...
 */
export async function getMediaInfoVersion(
  options?: MediaInfoOptions,
): Promise<string> {
  const binPath = findMediaInfoBin(options?.binPath);
  return new Promise((resolve, reject) => {
